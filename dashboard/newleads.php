...
{
	$e = $_SESSION['syt_email'];
	
	$check_id = mysql_query("SELECT * FROM `syt_registration` WHERE `email`='$e'");
	$qq = mysql_fetch_array($check_id);
	$t1 = $qq['category'];
	
	$cate = mysql_query("SELECT * FROM `syt_category` WHERE `category`='$t1'");
	$qq1=mysql_fetch_array($cate);
	$r=$qq1['id'];
	header("Location:../categories/lead_listing.php?catid=$r");
}
?>
<!DOCTYPE html>
...
<head>
	<meta charset="utf-8" />
	<title>Sell Your Time | New Leads</title>
