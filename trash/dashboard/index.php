...
<body class="fixed-top">
	...
	var DemoGMaps = function () {
     ...
     var mapGeocoding = function() {
		...
		<?php 
			if(mysql_num_rows($check_location1) == 0)
			{
			...
			else
			{
				while($rq = mysql_fetch_assoc($check_location1))
				{
					...
					$ert = mysql_query("SELECT * FROM `syt_location` WHERE `zipcode` = '$loc1'");
					$fet_ert = mysql_fetch_array($ert);
					
					 $lat =$fet_ert['latitude'];
					$log =$fet_ert['longitude'];
		?>
					map.addMarker({
					lat: <?php echo $lat;?>,
					lng: <?php echo $log;?>,
					...
					infoWindow: {
						content: '<span style="color:#678666"><?php echo $rq['category'];?></span>'
					}
				});
