...
// echo $category1." ".$subcategory;
// $sql = mysql_query("SELECT * FROM `syt_post` WHERE `category` like '$category' or `subcategory` like '$subcategory'");
$sql = mysql_query("SELECT * FROM `syt_post` WHERE  (`category` = '$category') or `subcategory` = '$keyword' or `zip` like '$location' or `subcategory` = '$subcategory' or `syt_title` like '$keyword'");
...
if($sql_num_rows == 0)
{
echo "<p style='margin-left:2%;'><font size='5'>Looking to find business leads under ".$category1."?<br>&nbsp;&nbsp; Please <a href='../register'>signup</a> or <a href='../register'>login</a> with us.</font></p>";
}
else
{
